...
from aug import aug_p
from datasets import FMLPRecDataset

sequential_data_list = ['Beauty', 'Sports_and_Outdoors', 'Toys_and_Games', 'Yelp', 'ml-1m']
...
def get_rating_matrix(data_name, seq_dic, max_item):
    ...
    if data_name in sequential_data_list:
        ...
        valid_rating_matrix = generate_rating_matrix_test(seq_dic['user_seq_eval'], seq_dic['num_users_eval'],
                                                          num_items)
...
def get_seq_dic(args):
    if args.data_name in sequential_data_list:
        ...
        args.train_data_file = args.data_dir + args.data_name +'_' +str(args.max_seq_length)+".txt"
        # 如果数据不存在就生成数据
        if not os.path.exists(args.train_data_file):
            aug_p(args.data_file, args.train_data_file,args.max_seq_length)
        # 训练集数据
        train_user_seq, train_max_item, _, _ = get_user_seqs(args.train_data_file)
        user_seq, max_item, valid_rating_matrix, test_rating_matrix = \
            get_user_seqs(args.data_file)
        seq_dic = {'user_seq': user_seq, 'valid_rating_matrix':valid_rating_matrix, 'test_rating_matrix':test_rating_matrix, 'train_user_seq': train_user_seq, 'train_max_item': train_max_item}
...
def get_dataloader(args, seq_dic):
    if args.data_name in sequential_data_list:
        if args.data_aug:
            train_dataset = FMLPRecDataset(args, seq_dic['train_user_seq'], data_type='train')
        else:
            train_dataset = FMLPRecDataset(args, seq_dic['user_seq'], data_type='train')
        train_sampler = RandomSampler(train_dataset)
        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.batch_size)

        eval_dataset = FMLPRecDataset(args, seq_dic['user_seq'],data_type='valid')
        ...
        eval_dataset = FMLPRecDataset(args, seq_dic['user_seq_eval'], data_type='session')
        eval_sampler = SequentialSampler(eval_dataset)
        eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.batch_size)

        test_dataset = FMLPRecDataset(args, seq_dic['user_seq_test'], data_type='session')
        test_sampler = SequentialSampler(test_dataset)
        test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=args.batch_size)
